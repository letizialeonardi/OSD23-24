 ...
 * I thread usano i semafori per sincronizzare la scrittura su un buffer di stampa in modo che risulti una serie di NTIMES 'HHO' corrispondente a H2O. Tale buffer viene poi stampato dal main */
#include <pthread.h>
#include <semaphore.h>
#include <stdio.h>
#include <stdlib.h>
...
/* variabili globali */
char stampa[3*NTIMES+1];				/* array dove vengono inseriti i caratteri da stampare (l'ultimo carattere serve per il terminatore di stringa */
int indice = 0; 					/* indice per l'array stampa */
...
int main (int argc, char **argv)
{
   ...
   /* Una volta che i 2 thread figli sono finiti, il main deve passare alla fase di stampa; OSSERVAZIONE: questa soluzione chiaramente non sarebbe corretta se i thread generatori fossero dei cicli senza fine! */
   stampa[3*NTIMES] = '\0'; 	 /* N.B. poiche' di due thread sono terminati non serve proteggere l'accesso all'array stampa */
   printf("I generatori hanno prodotto la seguente sequenza %s\n", stampa);  
