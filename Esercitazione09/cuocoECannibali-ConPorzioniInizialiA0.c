 ...
 * Il riempimento della pentola da parte del cuoco e' stato simulato con una sleep, cosi' come mangiare la porzione da parte di un cannibale.
 * Ogni thread cannibale torna al main il proprio numero d'ordine. */
/* LA DIFFERENZA RISPETTO ALL'ALTRA VERSIONE E' SEMPLICEMENTE IL VALORE INIZIALE DELLA VARIABILE CHE TIENE TRACCIA DI QUANTE PORZIONI SONO DISPONIBILI CHE IN QUESTO CASO E' POSTO ALL'INIZIO UGUALE A 0 */
#include <pthread.h>
#include <semaphore.h>
#include <stdio.h>
#include <stdlib.h>
...
int porzioni= 0;					/* numero di porzioni presenti nella pentola: all'inizio si suppone che NON ce ne siano e quindi il primo cannibale che arrivera' andra'  subito a svegliare il cuoco */
...
int main (int argc, char **argv)
{
   ...
   /* prima di creare i thread, andiamo ad inizializzare il semaforo VUOTA al va
lore 0 */
