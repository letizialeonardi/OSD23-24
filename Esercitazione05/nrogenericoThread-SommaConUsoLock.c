/* OBIETTIVO: generare un numero non noto di threads che eseguono la modifica di una variabile globale in un ciclo, all'interno del quale stampano il valore corrente di tale variebile, 
il loro numero d'ordine e il loro identificatore; il risultato voluto sarebbe quello che ogni thread dovrebbe eseguire il ciclo di modifica in mutua esclusione 
 ==> poiche' il ciclo di modifica è considerato una sezione critica, il risultato della esecuzione e' quello sperato */
#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
...
pthread_mutex_t mtx = PTHREAD_MUTEX_INITIALIZER; /* definiamo un mutex per proteggere la sezione critica rappresenrata dal ciclo for di incremento della variabile x */
...
void *Incremento(void *id) /* in questo caso alla START_ROUTINE e' stato dato un nome specifico relativo al 'compito' che i vari thread figli devono svolgere */
{
   ...
   /* proteggiamo la sezione critica */
