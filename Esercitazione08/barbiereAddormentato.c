...
void *eseguiBarbiere(void *id)
{
   ...
   for (i = 0; ; i++) /* while (true)  il barbiere DEVE essere un ciclo senza fine */
   {
	...
	/* il barbiere puo' prendere in carico un cliente: questo viene rappresentato dal fatto che facendo sedere il cliente sulla sedia del barbiere
      si libera un posto nella sala di attesa e chiaramente si va a risvegliare uno dei clienti in attesa */ 
...
void *eseguiCliente(void *id)
{
   ...
   if (FreeSeats > 0)
   {
   	FreeSeats--;
        ...
        pthread_mutex_unlock(&MUTEX);
        sem_wait(&BARBER);
        printf("CLIENTE-[Thread%d e identificatore %lu] SONO SULLA SEDIA DEL BARBIERE\n", *pi, pthread_self());
   }
   else
   {
   	/* fine sezione critica */
        pthread_mutex_unlock(&MUTEX);
        printf("CLIENTE-[Thread%d e identificatore %lu] NON HO TROVATO POSTO E QUINDI VADO A CASA\n", *pi, pthread_self());
   }
   printf("CLIENTE-[Thread%d e identificatore %lu] ARRIVATO A CASA\n", *pi, pthread_self());
