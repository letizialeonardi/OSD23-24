/* OBIETTIVO: generare un numero non noto di threads che eseguono il codice corrispondente all'esempio di PRODUTTORI E CONSUMATORI RISOLTO CON UNA SOLUZIONE CHE USA UN
MUTEX E DUE VARIABILI CONDIZIONE.
...
/* NOTA BENE: nelle funzioni/procedure entry DEPOSITA e PRELEVA si e' passato l'indice del Thread per rendere piu' chiare le stampe */
#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
typedef struct{
        char dato[5];    /* stringa che rappresenta il vero messaggio: per semplicita' ogni messaggio viene considerato di dimensione fissa e pari ad una stringa di 4 caratteri 
                         (piu' il terminatore di stringa)! */
        int iter;        /* valore della iterazione corrente */
                } Messaggio;

/* variabili globali */
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;     	/* semaforo di mutua esclusione per l'accesso a tutte le variabili condivise 
...
/* NOTA BENE: IL MESSAGGIO DEVE ESSERE COPIATO DALLA VARIABILE LOCALE messaggio DEL THREAD PRODUTTORE NEL BUFFER; 
RISULTA SCORRETTO INSERIRE NEL BUFFER SEMPLICEMENTE UN PUNTATORE AL MESSAGGIO POICHE' TALE MESSAGGIO POTREBBE POI 
ESSERE MODIFICATO E QUINDI NON ESSERE PIU' DISPONIBILE NELLA SUA 'FORMA' ORIGINALE PER GLI SCOPI DEL CONSUMATORE! */
	punt1 = (punt1 + 1) % N;
	inseriti++;
...
void PRELEVA(Messaggio *messaggio, int id)
{
	...
	/* NOTA BENE: IL MESSAGGIO DEVE ESSERE COPIATO NELLA VARIABILE LOCALE messaggio DEL THREAD CONSUMATORE; 
     RISULTA SCORRETTO TORNARE SEMPLICEMENTE UN PUNTATORE ALL'ELEMENTO DEL BUFFER POICHE' TALE ELEMENTO POTREBBE POI 
     ESSERE MODIFICATO E QUINDI NON ESSERE PIU' DISPONIBILE NELLA SUA 'FORMA' ORIGINALE PER GLI SCOPI DEL CONSUMATORE! */
        punt2 = (punt2 + 1) % N;
	inseriti--;
...
int main (int argc, char **argv)
{
   ...
   if (taskids == NULL)
   {
        perror("Problemi con l'allocazine dell'array taskids\n");
        exit(4);
   }
