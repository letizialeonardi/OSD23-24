/* OBIETTIVO: generare un numero non noto di threads che eseguono il codice corrispondente all'esempio di PRODUTTORI E CONSUMATORI risolto con i semafori. 
 * La dimensione del buffer e' definita dalla costante N (per semplicita'). 
 * Ogni singolo messaggio scambiato e' una stringa di lunghezza 4 corrispondente al numero d'ordine del thread produttore. 
...
char buffer[N][5];  					/* per semplicita' ogni messaggio viene considerato di dimensione fissa e pari ad una stringa di 4 caratteri caratteri (piu' il terminatore di stringa)! */ 
int punt1 = 0, punt2 = 0;			 	/* punt1 viene usato dai produttori per inserire e punt2 viene usato dai consumatori per prelevare */

void INSERIMENTO(char messaggio[5])
{
	strcpy(buffer[punt1], messaggio); /* corrisponde all'istruzione buffer[punt1] = messaggio; delle slide */	
...
void PRELIEVO(char *messaggio)
{
        strcpy(messaggio, buffer[punt2]); /* corrisponde all'istruzione messaggio = buffer[punt2]; delle slide */
...
void *eseguiConsumatore(void *id)
{
   ...
   for (i = 0; i < NTIMES; i++) /* while (true)  i produttori dovrebbero essere cicli senza fine */
   {
	...
	PRELIEVO(messaggio);
	/* NOTA BENE: IL MESSAGGIO DEVE ESSERE COPIATO NELLA VARIABILE LOCALE messaggio DEL THREAD CONSUMATORE; RISULTA SCORRETTO TORNARE SEMPLICEMENTE UN PUNTATORE ALL'ELEMENTO DEL BUFFER POICHE' TALE ELEMENTO POTREBBE POI ESSERE MODIFICATO E QUINDI NON ESSERE PIU' DISPONIBILE NELLA SUA 'FORMA' ORIGINALE PER GLI SCOPI DEL CONSUMATORE! */
	printf("CONSUMATORE-Thread%d e identificatore %lu ha ricevuto %s\n", *pi, pthread_self(), messaggio);
   	/* fine sezione critica */
