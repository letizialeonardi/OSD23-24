...
pthread_cond_t *disponibili;				/* array dinamico di variabili conditions, una per ogni utente */
...
{
	/* N.B. L'indice i del thread utente serve per sapere su quale variabile condizione ci si deve sospendere */
	pthread_mutex_lock(&mutex);     /* simulazione di inizio procedura entry del monitor */
        while (licenze_correnti < count) /* si deve usare un while perche' quando si rilasciano le licenze non si possono conoscenre le singole richieste fatte da altri utenti */
	{
...
{
	/* N.B. L'indice i del thread utente serve per sapere su quale variabile condizione non si deve fare la signal (in realta' non servirebbe, ma risulta piu' chiaro il codice */
	int j;
        pthread_mutex_lock(&mutex);     /* simulazione di inizio procedura entry del monitor */
   	...
   	printf("Thread%d e identificatore %lu STA RILASCIANDO %d licenze\n", i, pthread_self(), count);
		...
		if (j != i)
                	pthread_cond_signal(&disponibili[j]); /* vengono segnalate tutte le variabili condition, esclusa quella dell'utente corrente: nel caso, ci siano utenti sospesi verranno risvegliati, altrimenti la signal non avra' effetto */ 
        pthread_mutex_unlock(&mutex);   /* simulazione di termine procedura entry del monitor */
}
...
int main (int argc, char **argv)
{
   ...
   if (disponibili == NULL)
   {
        perror("Problemi con l'allocazione dell'array disponibili\n");
        exit(5);
   }
